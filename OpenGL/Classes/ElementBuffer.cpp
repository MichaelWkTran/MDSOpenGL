...
CElementBuffer::CElementBuffer(const std::vector<GLuint> _GLuIndicies)
{
    glGenBuffers(1, &m_GLuID);
...
void CElementBuffer::SetIndicies(const std::vector<GLuint> _vIndicies)
{
    m_vIndicies = _vIndicies;

    Bind();
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_vIndicies.size() * sizeof(GLuint), m_vIndicies.data(), GL_STATIC_DRAW);
    Unbind();
}
