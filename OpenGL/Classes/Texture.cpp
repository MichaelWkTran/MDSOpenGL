...
/*static */std::map<GLuint, CTexture*> CTextureManager::m_mapTextures;
...
/*static */CTexture* CTextureManager::Insert(const char* _pType, GLuint&& _GLuSlot, GLenum&& _GLeTarget/* = GL_TEXTURE_2D*/, const char* _pName/* = ""*/)
{
	CTexture* pTexture = new CTexture(_pName, _pType, std::move(_GLeTarget), std::move(_GLuSlot));
	m_mapTextures.insert(std::pair<GLuint, CTexture*>(pTexture->GetID(), pTexture));
...
CTexture::CTexture(const char* _pName, const char* _pType, GLenum&& _GLeTarget, GLuint&& _GLuSlot)
{
...
void CTexture::Uniform(GLuint _GluShaderID, std::string _strUniformName)
{
	Bind();
