...
CCubeMap::CCubeMap(CShader* _pShader, float _fSize, const char* _pTextureDirectories[6])
{
	m_Mesh.m_pShader = _pShader;
	gm::GenerateFlatCube(m_Mesh, glm::vec3(1.0f,1.0f,1.0f) * _fSize);
	
	//Invert Faces
	std::vector<GLuint> vIndicies = m_Mesh.GetIndicies();
	for (int i = 0; i < vIndicies.size(); i += 3)
	{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		GLuint GLuLoadedComponents = (iImageComponents == 4) ? GL_RGBA : GL_RGB;
		glTexImage2D
		(
...
void CCubeMap::Draw(const CCamera& _Camera)
{
